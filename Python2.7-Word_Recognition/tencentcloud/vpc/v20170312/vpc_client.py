...
class VpcClient(AbstractClient):
    ...
    def CreateDefaultVpc(self, request):
        ...
        預設VPC适用於快速入門和啓動公共實例，您可以像使用任何其他VPC一樣使用預設VPC。如果你想創建標準VPC，即指定VPC名稱、VPC網段、子網網段、子網可用區，請使用常規創建VPC介面（CreateVpc）
    ...
    def CreateSecurityGroupPolicies(self, request):
        ...
        * Version安全組規則版本號，用戶每次更新安全規則版本會自動加1，防止你更新的路由規則已過期，不填不考慮沖突。
