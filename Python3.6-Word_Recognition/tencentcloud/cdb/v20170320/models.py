...
class CloseWanServiceRequest(AbstractModel):
    ...
    def __init__(self):
        """
        :param InstanceId: 實例ID，格式如：cdb-c1nl9rpv，與雲資料庫控制台頁面中顯示的實例ID相同，可使用[查詢實例清單](https://cloud.tencent.com/document/api/236/15872) 介面獲取，其值爲輸出參數中欄位 InstanceId 的值。
        :type InstanceId: str
        """
        self.InstanceId = None
...
class CreateDBInstanceHourRequest(AbstractModel):
    ...
    def __init__(self):
        ...
        :type Volume: int
        :param EngineVersion: MySQL版本，值包括：5.5、5.6和5.7，請使用[獲取雲資料庫可售賣規格](https://cloud.tencent.com/document/api/236/17229)介面獲取可創建的實例版本
        ...
        :param ProjectId: 項目ID，不填爲預設項目。請使用[查詢項目清單](https://cloud.tencent.com/document/product/378/4400)介面獲取項目ID
        :type ProjectId: int
        :param Zone: 可用區訊息，該參數缺省時，系統會自動選擇一個可用區，請使用[獲取雲資料庫可售賣規格](https://cloud.tencent.com/document/api/236/17229)介面獲取可創建的可用區
        :type Zone: str
        :param MasterInstanceId: 實例ID，購買只讀實例或者災備實例時必填，該欄位表示只讀實例或者災備實例的主實例ID，請使用[查詢實例清單](https://cloud.tencent.com/document/api/236/15872)介面查詢雲資料庫實例ID
        ...
        :type Password: str
        :param ParamList: 參數清單，參數格式如ParamList.0.Name=auto_increment&ParamList.0.Value=1。可通過[查詢預設的可設置參數清單](https://cloud.tencent.com/document/api/236/32662)查詢支援設置的參數
...
class CreateDBInstanceRequest(AbstractModel):
    ...
    def __init__(self):
        ...
        :param InstanceRole: 實例類型，預設爲 master，支援值包括：master-表示主實例，dr-表示災備實例，ro-表示只讀實例
        :type InstanceRole: str
        :param MasterInstanceId: 實例ID，購買只讀實例時必填，該欄位表示只讀實例的主實例ID，請使用[查詢實例清單](https://cloud.tencent.com/document/api/236/15872)介面查詢雲資料庫實例ID
        :type MasterInstanceId: str
        :param EngineVersion: MySQL版本，值包括：5.5、5.6和5.7，請使用[獲取雲資料庫可售賣規格](https://cloud.tencent.com/document/api/236/17229)介面獲取可創建的實例版本
        :type EngineVersion: str
        :param Password: 設置root帳号密碼，密碼規則：8-64個字元，至少包含字母、數字、字元（支援的字元：_+-&=!@#$%^*()）中的兩種，購買主實例時可指定該參數，購買只讀實例或者災備實例時指定該參數無意義
...
class DeleteBackupRequest(AbstractModel):
    ...
    def __init__(self):
        ...
        :param InstanceId: 實例ID，格式如：cdb-c1nl9rpv。與雲資料庫控制台頁面中顯示的實例ID相同。
        :type InstanceId: str
        :param BackupId: 備份任務ID。該任務ID爲[創建雲資料庫備份](https://cloud.tencent.com/document/api/236/15844)介面返回的任務ID。
        :type BackupId: int
        """
        self.InstanceId = None
        self.BackupId = None
...
class DescribeDBInstancesRequest(AbstractModel):
    ...
    def __init__(self):
        """
        :param ProjectId: 項目ID，可使用[查詢項目清單](https://cloud.tencent.com/document/product/378/4400)介面查詢項目ID
        :type ProjectId: int
...
class InquiryPriceUpgradeInstancesRequest(AbstractModel):
    ...
    def __init__(self):
        """
        :param InstanceId: 實例ID，格式如：cdb-c1nl9rpv或者cdbro-c1nl9rpv。與雲資料庫控制台頁面中顯示的實例ID相同，可使用[查詢實例清單](https://cloud.tencent.com/document/api/236/15872) 介面獲取，其值爲輸出參數中欄位 InstanceId 的值
        :type InstanceId: str
...
class ModifyDBInstanceProjectRequest(AbstractModel):
    ...
    def __init__(self):
        """
        :param InstanceIds: 實例ID數組，格式如：cdb-c1nl9rpv，與雲資料庫控制台頁面中顯示的實例ID相同，可使用[查詢實例清單](https://cloud.tencent.com/document/api/236/15872) 介面獲取，其值爲輸出參數中欄位 InstanceId 的值。
        :type InstanceIds: list of str
...
class RenewDBInstanceRequest(AbstractModel):
    ...
    def __init__(self):
        """
        :param InstanceId: 待續約的實例ID，格式如：cdb-c1nl9rpv，與雲資料庫控制台頁面中顯示的實例ID相同，可使用[查詢實例清單](https://cloud.tencent.com/document/api/236/15872)
        :type InstanceId: str
...
class UpgradeDBInstanceEngineVersionRequest(AbstractModel):
    ...
    def __init__(self):
        ...
        :type EngineVersion: str
        :param WaitSwitch: 切換訪問新實例的方式，預設爲0。支援值包括：0-立刻切換，1-時間窗切換；當該值爲1時，升級中過程中，切換訪問新實例的流程将會在時間窗内進行，或者用戶主動調用介面[切換訪問新實例](https://cloud.tencent.com/document/product/236/15864)觸發該流程
        :type WaitSwitch: int
        """
        self.InstanceId = None
        self.EngineVersion = None
...
class UpgradeDBInstanceEngineVersionResponse(AbstractModel):
    ...
    def __init__(self):
        """
        :param AsyncRequestId: 異步任務ID，可使用[查詢異步任務的執行結果](https://cloud.tencent.com/document/api/236/20410)獲取其執行情況。
        :type AsyncRequestId: str
        :param RequestId: 唯一請求 ID，每次請求都會返回。定位問題時需要提供該次請求的 RequestId。
        :type RequestId: str
        """
...
class UpgradeDBInstanceRequest(AbstractModel):
    ...
    def __init__(self):
        ...
        :type DeployMode: int
        :param SlaveZone: 備庫1的可用區訊息，預設和實例的Zone參數一緻，升級主實例爲多可用區佈署時可指定該參數，升級只讀實例或者災備實例時指定該參數無意義。可通過[獲取雲資料庫可售賣規格](https://cloud.tencent.com/document/product/236/17229)介面查詢支援的可用區
        :type SlaveZone: str
