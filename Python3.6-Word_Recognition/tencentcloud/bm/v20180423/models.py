...
class DescribeDeviceInventoryRequest(AbstractModel):
    ...
    def __init__(self):
        ...
        :param SubnetId: 子網ID
        :type SubnetId: str
        :param CpuId: CpuId，自定義機型時需傳入
        :type CpuId: int
        :param DiskType: 硬碟類型，自定義機型時需傳入
        :type DiskType: str
        :param DiskSize: 單塊硬碟大小，自定義機型時需傳入
        :type DiskSize: int
        :param DiskNum: 硬碟數量，自定義機型時需傳入
        :type DiskNum: int
        :param Mem: 内存總大小，自定義機型時需傳入
        :type Mem: int
        :param HaveRaidCard: 是否支援raid，自定義機型時需傳入
