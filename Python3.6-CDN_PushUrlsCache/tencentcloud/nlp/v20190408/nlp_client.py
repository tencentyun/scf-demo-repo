...
class NlpClient(AbstractClient):
    ...
    def ChatBot(self, request):
        """閑聊服務基于 領先的NLP引擎能力、數據運算能力和千億級網際網路語料數據的支援，同時內建了廣泛的知識問答能力，可實現上百種自定義屬性配置，以及兒童語言風格及說話方式，從而讓聊天變得更睿智、簡單和有趣。
    ...
    def LexicalAnalysis(self, request):
        ...
        所有的功能均基于千億級大規模網際網路語料進行持續疊代更新，以保證效果不斷提升，用戶無需擔心新詞發現、歧義消除、調用效能等問題。目前詞法分析已經在泛網際網路、金融、政務等不同垂直領域提供業務支援，并取得良好的效果。
    ...
    def SentenceEmbedding(self, request):
        ...
        該句向量服務由 知文自然語言處理團隊聯合 AI Lab共同打造，基于千億級大規模網際網路語料并采用AI Lab自研的DSG算法訓練而成，在 内部諸多業務的NLP任務上實測效果顯著。
    ...
    def SentenceSimilarity(self, request):
        ...
        鑒于文本相似度是一個應用非常廣泛的功能， 知文自然語言處理團隊在深度神經網絡模型的基礎上，專門針對文本相似任務進行了優化，并持續疊代更新。基于文本相似度，可以輕松實現諸如文本去重、相似推薦等功能。
    ...
    def SentimentAnalysis(self, request):
        ...
        該功能基于基于千億級大規模網際網路語料和LSTM、BERT等深度神經網絡模型進行訓練，并持續疊代更新，以保證效果不斷提升。
    ...
    def TextClassification(self, request):
        ...
        - 新聞領域分類體系，包括37個一級分類類目，285個二級分類（詳細請見 [類目體系映射表](https://cloud.taifucloud.com/document/product/271/36459)），已應用于 新聞的文章分類。
    ...
    def TextCorrection(self, request):
        ...
        此功能是基于千億級大規模網際網路語料和LSTM、BERT等深度神經網絡模型進行訓練，并持續疊代更新，以保證效果不斷提升，是搜索引擎、語音識别、内容審核等功能更好運作的基礎之一。
    ...
    def WordEmbedding(self, request):
        ...
        該詞向量服務由 知文自然語言處理團隊聯合 AI Lab共同打造。使用的詞向量基于千億級大規模網際網路語料并采用AI Lab自研的DSG算法訓練而成，開源的詞向量包含800多萬中文詞匯，在函蓋率、新鮮度及準确性等三方面效能突出。
