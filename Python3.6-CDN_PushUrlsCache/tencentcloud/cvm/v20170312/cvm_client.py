...
class CvmClient(AbstractClient):
    ...
    def AssociateSecurityGroups(self, request):
        ...
        * 實例操作結果可以通過調用 [DescribeInstances](https://cloud.taifucloud.com/document/api/213/15728#.E7.A4.BA.E4.BE.8B3-.E6.9F.A5.E8.AF.A2.E5.AE.9E.E4.BE.8B.E7.9A.84.E6.9C.80.E6.96.B0.E6.93.8D.E4.BD.9C.E6.83.85.E5.86.B5) 介面查詢，如果實例的最新操作狀态(LatestOperationState)爲“SUCCESS”，則代表操作成功。
    ...
    def CreateDisasterRecoverGroup(self, request):
        """本介面 (CreateDisasterRecoverGroup)用于創建[分散置放群組](https://cloud.taifucloud.com/document/product/213/15486)。創建好的置放群組，可在[創建實例](https://cloud.taifucloud.com/document/api/213/15730)時指定。
    ...
    def DeleteDisasterRecoverGroups(self, request):
        """本介面 (DeleteDisasterRecoverGroups)用于删除[分散置放群組](https://cloud.taifucloud.com/document/product/213/15486)。只有空的置放群組才能被删除，非空的群組需要先銷毀組内所有雲伺服器，才能執行删除操作，不然會産生删除置放群組失敗的錯誤。
    ...
    def DeleteImages(self, request):
        ...
        * 當[映像狀态](https://cloud.taifucloud.com/document/product/213/15753#Image)爲`創建中`和`使用中`時, 不允許删除。映像狀态可以通過[DescribeImages](https://cloud.taifucloud.com/document/api/213/9418)獲取。
    ...
    def DescribeDisasterRecoverGroupQuota(self, request):
        """本介面 (DescribeDisasterRecoverGroupQuota)用于查詢[分散置放群組](https://cloud.taifucloud.com/document/product/213/15486)配額。
    ...
    def DescribeDisasterRecoverGroups(self, request):
        """本介面 (DescribeDisasterRecoverGroups)用于查詢[分散置放群組](https://cloud.taifucloud.com/document/product/213/15486)訊息。
    ...
    def DescribeInstanceTypeConfigs(self, request):
        ...
        * 可以根據`zone`、`instance-family`來查詢實例機型配置。過濾條件詳見過濾器[`Filter`](https://cloud.taifucloud.com/document/api/213/15753#Filter)。
    ...
    def DescribeReservedInstances(self, request):
        ...
        :type request: :class:`taifucloudcloud.cvm.v20170312.models.DescribeReservedInstancesRequest`
        :rtype: :class:`taifucloudcloud.cvm.v20170312.models.DescribeReservedInstancesResponse`
    ...
    def DescribeReservedInstancesOfferings(self, request):
        ...
        :type request: :class:`taifucloudcloud.cvm.v20170312.models.DescribeReservedInstancesOfferingsRequest`
        :rtype: :class:`taifucloudcloud.cvm.v20170312.models.DescribeReservedInstancesOfferingsResponse`
    ...
    def DisassociateInstancesKeyPairs(self, request):
        ...
        * 只支援[`STOPPED`](https://cloud.taifucloud.com/document/product/213/15753#InstanceStatus)狀态的`Linux`作業系統的實例。
    ...
    def ImportKeyPair(self, request):
        ...
        * 本介面的功能是将金鑰對導入到用戶帳戶，并不會自動綁定到實例。如需綁定可以使用[AssociasteInstancesKeyPair](https://cloud.taifucloud.com/document/api/213/9404)介面。
        * 需指定金鑰對名稱以及該金鑰對的公鑰文本。
    ...
    def InquiryPriceResetInstance(self, request):
        ...
        * 目前只支援[系統盤類型](https://cloud.taifucloud.com/document/api/213/15753#SystemDisk)是`CLOUD_BASIC`、`CLOUD_PREMIUM`、`CLOUD_SSD`類型的實例使用該介面實現`Linux`和`Windows`作業系統切換的重裝詢價。
    ...
    def InquiryPriceResetInstancesInternetMaxBandwidth(self, request):
        ...
        * 不同機型頻寬上限範圍不一緻，具體限制詳見[公網頻寬上限](https://cloud.taifucloud.com/document/product/213/12523)。
        * 對于`BANDWIDTH_PREPAID`計費方式的頻寬，需要輸入參數`StartTime`和`EndTime`，指定調整後的頻寬的生效時間段。在這種場景下目前不支援調小頻寬，會涉及扣費，請确保帳戶餘額充足。可通過[`DescribeAccountBalance`](https://cloud.taifucloud.com/document/product/555/20253)介面查詢帳戶餘額。
    ...
    def InquiryPriceResetInstancesType(self, request):
        ...
        * 目前只支援[系統盤類型](https://cloud.taifucloud.com/document/product/213/15753#SystemDisk)是`CLOUD_BASIC`、`CLOUD_PREMIUM`、`CLOUD_SSD`類型的實例使用該介面進行調整機型詢價。
        * 目前不支援[CDH](https://cloud.taifucloud.com/document/product/416)實例使用該介面調整機型詢價。
        * 對于包年包月實例，使用該介面會涉及扣費，請确保帳戶餘額充足。可通過[`DescribeAccountBalance`](https://cloud.taifucloud.com/document/product/555/20253)介面查詢帳戶餘額。
    ...
    def InquiryPriceResizeInstanceDisks(self, request):
        ...
        * 目前只支援擴容非彈性數據盤（[`DescribeDisks`](https://cloud.taifucloud.com/document/api/362/16315)介面返回值中的`Portable`爲`false`表示非彈性）詢價，且[數據盤類型](https://cloud.taifucloud.com/document/product/213/15753#DataDisk)爲：`CLOUD_BASIC`、`CLOUD_PREMIUM`、`CLOUD_SSD`。
    ...
    def InquiryPriceRunInstances(self, request):
        """本介面(InquiryPriceRunInstances)用于創建實例詢價。本介面僅允許針對購買限制範圍内的實例配置進行詢價, 詳見：[創建實例](https://cloud.taifucloud.com/document/api/213/15730)。
    ...
    def ModifyDisasterRecoverGroupAttribute(self, request):
        """本介面 (ModifyDisasterRecoverGroupAttribute)用于修改[分散置放群組](https://cloud.taifucloud.com/document/product/213/15486)屬性。
    ...
    def ModifyInstancesProject(self, request):
        ...
        * 項目爲一個虛拟概念，用戶可以在一個帳戶下面建立多個項目，每個項目中管理不同的資源；将多個不同實例分屬到不同項目中，後續使用 [`DescribeInstances`](https://cloud.taifucloud.com/document/api/213/15728)介面查詢實例，項目ID可用于過濾結果。
        * 綁定負載均衡的實例不支援修改實例所屬項目，請先使用[`DeregisterInstancesFromLoadBalancer`](https://cloud.taifucloud.com/document/api/214/1258)介面解綁負載均衡。
        [^_^]: # ( 修改實例所屬項目會自動解關聯實例原來關聯的安全組，修改完成後可使用[`ModifyInstancesAttribute`](https://cloud.taifucloud.com/document/api/213/15739)介面關聯安全組。)
        * 支援批次操作。每次請求批次實例的上限爲100。
        * 實例操作結果可以通過調用 [DescribeInstances](https://cloud.taifucloud.com/document/api/213/15728#.E7.A4.BA.E4.BE.8B3-.E6.9F.A5.E8.AF.A2.E5.AE.9E.E4.BE.8B.E7.9A.84.E6.9C.80.E6.96.B0.E6.93.8D.E4.BD.9C.E6.83.85.E5.86.B5) 介面查詢，如果實例的最新操作狀态(LatestOperationState)爲“SUCCESS”，則代表操作成功。
    ...
    def PurchaseReservedInstancesOffering(self, request):
        ...
        :type request: :class:`taifucloudcloud.cvm.v20170312.models.PurchaseReservedInstancesOfferingRequest`
        :rtype: :class:`taifucloudcloud.cvm.v20170312.models.PurchaseReservedInstancesOfferingResponse`
    ...
    def RenewHosts(self, request):
        ...
        * 續約時請确保帳戶餘額充足。可通過[`DescribeAccountBalance`](https://cloud.taifucloud.com/document/product/555/20253)介面查詢帳戶餘額。
    ...
    def ResetInstance(self, request):
        ...
        * 密碼不指定将會通過站内信下發随機密碼。
        * 目前只支援[系統盤類型](https://cloud.taifucloud.com/document/api/213/9452#SystemDisk)是`CLOUD_BASIC`、`CLOUD_PREMIUM`、`CLOUD_SSD`類型的實例使用該介面實現`Linux`和`Windows`作業系統切換。
    ...
    def ResetInstancesType(self, request):
        ...
        * 目前不支援[CDH](https://cloud.taifucloud.com/document/product/416)實例使用該介面調整機型。對于包年包月實例，使用該介面會涉及扣費，請确保帳戶餘額充足。可通過[`DescribeAccountBalance`](https://cloud.taifucloud.com/document/product/555/20253)介面查詢帳戶餘額。
        * 本介面爲異步介面，調整實例配置請求發送成功後會返回一個RequestId，此時操作并未立即完成。實例操作結果可以通過調用 [DescribeInstances](https://cloud.taifucloud.com/document/api/213/15728#.E7.A4.BA.E4.BE.8B3-.E6.9F.A5.E8.AF.A2.E5.AE.9E.E4.BE.8B.E7.9A.84.E6.9C.80.E6.96.B0.E6.93.8D.E4.BD.9C.E6.83.85.E5.86.B5) 介面查詢，如果實例的最新操作狀态(LatestOperationState)爲“SUCCESS”，則代表調整實例配置操作成功。
    ...
    def ResizeInstanceDisks(self, request):
        ...
        * 目前只支援擴容非彈性數據盤（[`DescribeDisks`](https://cloud.taifucloud.com/document/api/362/16315)介面返回值中的`Portable`爲`false`表示非彈性），且[數據盤類型](https://cloud.taifucloud.com/document/api/213/15753#DataDisk)爲：`CLOUD_BASIC`、`CLOUD_PREMIUM`、`CLOUD_SSD`和[CDH](https://cloud.taifucloud.com/document/product/416)實例的`LOCAL_BASIC`、`LOCAL_SSD`類型數據盤。
        * 對于包年包月實例，使用該介面會涉及扣費，請确保帳戶餘額充足。可通過[`DescribeAccountBalance`](https://cloud.taifucloud.com/document/product/555/20253)介面查詢帳戶餘額。
        * 目前只支援擴容一塊數據盤。
        * 實例操作結果可以通過調用 [DescribeInstances](https://cloud.taifucloud.com/document/api/213/15728#.E7.A4.BA.E4.BE.8B3-.E6.9F.A5.E8.AF.A2.E5.AE.9E.E4.BE.8B.E7.9A.84.E6.9C.80.E6.96.B0.E6.93.8D.E4.BD.9C.E6.83.85.E5.86.B5) 介面查詢，如果實例的最新操作狀态(LatestOperationState)爲“SUCCESS”，則代表操作成功。
    ...
    def RunInstances(self, request):
        ...
        * 實例創建成功後将自動開機啓動，[實例狀态](https://cloud.taifucloud.com/document/product/213/15753#InstanceStatus)變爲“運作中”。
        ...
        * 本介面爲異步介面，當創建實例請求下發成功後會返回一個實例`ID`清單和一個`RequestId`，此時創建實例操作并未立即完成。在此期間實例的狀态将會處于“PENDING”，實例創建結果可以通過調用 [DescribeInstances](https://cloud.taifucloud.com/document/api/213/15728)  介面查詢，如果實例狀态(InstanceState)由“PENDING”變爲“RUNNING”，則代表實例創建成功，“LAUNCH_FAILED”代表實例創建失敗。
