...
class CvmClient(AbstractClient):
    ...
    def DeleteDisasterRecoverGroups(self, request):
        """本介面 (DeleteDisasterRecoverGroups)用於删除[分散置放群組](https://cloud.taifucloud.com/document/product/213/15486)。只有空的置放群組才能被删除，非空的群組需要先銷毀組内所有雲伺服器，才能執行删除操作，不然會産生删除置放群組失敗的錯誤。
    ...
    def DescribeInstanceVncUrl(self, request):
        """本介面 ( DescribeInstanceVncUrl ) 用於查詢實例管理終端網址，獲取的網址可用於實例的 VNC 登入。
    ...
    def InquiryPriceResetInstance(self, request):
        """本介面 (InquiryPriceResetInstance) 用於重裝實例詢價。
    ...
    def InquiryPriceResetInstancesInternetMaxBandwidth(self, request):
        ...
        * 對於`BANDWIDTH_PREPAID`計費方式的頻寬，需要輸入參數`StartTime`和`EndTime`，指定調整後的頻寬的生效時間段。在這種場景下目前不支援調小頻寬，會涉及扣費，請确保帳戶餘額充足。可通過[`DescribeAccountBalance`](https://cloud.taifucloud.com/document/product/555/20253)介面查詢帳戶餘額。
    ...
    def InquiryPriceResetInstancesType(self, request):
        ...
        * 對於包年包月實例，使用該介面會涉及扣費，請确保帳戶餘額充足。可通過[`DescribeAccountBalance`](https://cloud.taifucloud.com/document/product/555/20253)介面查詢帳戶餘額。
    ...
    def ModifyInstancesProject(self, request):
        ...
        * 項目爲一個虛拟概念，用戶可以在一個帳戶下面建立多個項目，每個項目中管理不同的資源；将多個不同實例分屬到不同項目中，後續使用 [`DescribeInstances`](https://cloud.taifucloud.com/document/api/213/15728)介面查詢實例，項目ID可用於過濾結果。
    ...
    def PurchaseReservedInstancesOffering(self, request):
        """本介面(PurchaseReservedInstancesOffering)用於用戶購買一個或者多個指定配置的預留實例
    ...
    def ResetInstancesPassword(self, request):
        ...
        *如果是修改系統管理雲密碼：實例的作業系統不同，管理員帳號也會不一樣(`Windows`爲`Administrator`，`Ubuntu`爲`ubuntu`，其它系統爲`root`)。
        * 重置處於運作中狀态的實例密碼，需要設置關機參數`ForceStop`爲`TRUE`。如果沒有顯式指定強制關機參數，則只有處於關機狀态的實例才允許執行重置密碼操作。
    ...
    def ResetInstancesType(self, request):
        ...
        * 目前不支援[CDH](https://cloud.taifucloud.com/document/product/416)實例使用該介面調整機型。對於包年包月實例，使用該介面會涉及扣費，請确保帳戶餘額充足。可通過[`DescribeAccountBalance`](https://cloud.taifucloud.com/document/product/555/20253)介面查詢帳戶餘額。
        * 本介面爲異步介面，調整實例配置請求發送成功後會返回一個RequestId，此時操作並未立即完成。實例操作結果可以通過調用 [DescribeInstances](https://cloud.taifucloud.com/document/api/213/15728#.E7.A4.BA.E4.BE.8B3-.E6.9F.A5.E8.AF.A2.E5.AE.9E.E4.BE.8B.E7.9A.84.E6.9C.80.E6.96.B0.E6.93.8D.E4.BD.9C.E6.83.85.E5.86.B5) 介面查詢，如果實例的最新操作狀态(LatestOperationState)爲“SUCCESS”，則代表調整實例配置操作成功。
    ...
    def RunInstances(self, request):
        ...
        * 本介面爲異步介面，當創建實例請求下發成功後會返回一個實例`ID`清單和一個`RequestId`，此時創建實例操作並未立即完成。在此期間實例的狀态将會處於“PENDING”，實例創建結果可以通過調用 [DescribeInstances](https://cloud.taifucloud.com/document/api/213/15728)  介面查詢，如果實例狀态(InstanceState)由“PENDING”變爲“RUNNING”，則代表實例創建成功，“LAUNCH_FAILED”代表實例創建失敗。
