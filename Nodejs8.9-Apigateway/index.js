...
const router = {
   ...
   * @return {boolean}
   */
  captures: async function(path, pathReg, cb) {
    if (this.match(path, pathReg)) {
      ...
      if (cb && cb instanceof Function) return cb(path, event)
      if (cb && cb instanceof Promise) return await cb(path, event)
    } else {
      return `路由器传入路径(${path})和注册路径(${pathReg})不匹配`
    }
  }
}
...
exports.main_handler = async (event, context, callback) => {
  ...
  const res = await router.captures(
    path,
    '/node8/article/:articleId',
    (path, params) => {
