...
class IirClient(AbstractClient):
    ...
    def RecognizeProduct(self, request):
        """商品識别，使用 掃一掃識物同款技術，基於人工智慧技術、海量訓練圖片、億級商品庫，可以實現全函蓋、細粒度、高準确率的商品識别和商品推薦功能。 本服務可以識别出圖片中的主體位置、主體商品類型，函蓋億級SKU，輸出具體商品的價格、型號等詳細訊息。 客戶無需自建商品庫，即可快速實現商品識别、拍照搜商品等功能。

        目前“商品識别”爲公測服務，需要申請、開通後方可使用。請在[服務開通申請表](https://cloud.taifucloud.com/apply/p/y1q2mnf0vdl) 中填寫詳細訊息和需求，如果通過審核，我們将會在2個工作日内與您聯系，並開通服務。 公測期間，本服務免費提供最高2QPS，收費模式和標準會在正式版上線前通過站内信、簡訊通知客戶。如果需要提升並發，請與我們聯系洽談。

        注意：本文件爲公測版本，僅适用於功能體驗和測試，正式業務接入請等待正式版。正式版的輸入、輸出可能會與公測版存在少量差異。
