...
hi：印地語
        :type Source: str
        :param Target: 目標語言，各源語言的目標語言支援清單如下
...
<li>hi（印地語）：en（英語）</li>
        ...
        :param ProjectId: 項目ID，可以根據控制台-賬號中心-項目管理中的配置填寫，如無配置請填寫預設項目ID:0
        :type ProjectId: int
        :param SourceTextList: 待翻譯的文本清單，批次介面可以以數組方式在一次請求中填寫多個待翻譯文本。文本統一使用utf-8格式編碼，非utf-8格式編碼字元會翻譯失敗，請傳入有效文本，html標記等非常規翻譯文本可能會翻譯失敗。單次請求的文本長度總和需要低於2000。
...
class TextTranslateRequest(AbstractModel):
    ...
    def __init__(self):
        """
        :param SourceText: 待翻譯的文本，文本統一使用utf-8格式編碼，非utf-8格式編碼字元會翻譯失敗，請傳入有效文本，html標記等非常規翻譯文本可能會翻譯失敗。單次請求的文本長度需要低於2000。
        :type SourceText: str
        :param Source: 源語言，支援：
auto：自動識别（識别爲一種語言）
zh：簡體中文
...
<li>hi（印地語）：en（英語）</li>
        ...
        :param ProjectId: 項目ID，可以根據控制台-賬號中心-項目管理中的配置填寫，如無配置請填寫預設項目ID:0
        :type ProjectId: int
        :param UntranslatedText: 用來標記不希望被翻譯的文本内容，如句子中的特殊符號、人名、地名等；每次請求只支援配置一個不被翻譯的單詞；僅支援配置人名、地名等名詞，不要配置動詞或短語，否則會影響翻譯結果。
