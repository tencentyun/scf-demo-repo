...
class TiiaClient(AbstractClient):
    ...
    def AssessQuality(self, request):
        """評估輸入圖片在視覺上的質量，從多個方面評估，並同時給出綜合的、客觀的清晰度評分，和主觀的美觀度評分。
        >
    ...
    def DetectCelebrity(self, request):
        """傳入一張圖片，可以識别圖片中包含的人物是否爲公衆人物，如果是，輸出人物的姓名、基本訊息、臉部坐標。
    ...
    def DetectDisgust(self, request):
        ...
        通過惡心圖片識别, 可以判斷一張圖片是否令人惡心, 同時給出它屬於的潛在類别, 讓您能夠過濾掉使人不愉快的圖片。
        >
        - 公共參數中的簽名方式必須指定爲V3版本，即配置SignatureMethod參數爲TC3-HMAC-SHA256。
    ...
    def DetectLabel(self, request):
        """圖像標簽利用深度學習技術、海量訓練數據，可以對圖片進行智慧分類、物體識别等。

        目前支援8個大類、六十多個子類、數千個標簽。涵蓋各種日常場景、動植物、物品、美食、卡證等。具體分類請見[圖像分析常見問題功能與限制相關](https://cloud.taifucloud.com/document/product/865/39164)。

        圖像標簽提供三個版本供選擇：

        • 攝像頭版：針對搜索、手機攝像頭照片進行優化，涵蓋大量卡證、日常物品、二維碼條形碼。

        • 相冊版：針對手機相冊、網盤進行優化，去除相冊和網盤中不常見的標簽，針對相冊常見圖片類型（人像、日常活動、日常物品等）識别效果更好。

        • 網絡版：針對網絡圖片進行優化，涵蓋標簽更多，滿足長尾識别需求。

        每個産品的圖像類型都有獨特性，建議在接入初期，對三個版本進行對比評估後選擇合适的版本使用。

        爲了方便使用、減少圖片傳輸次數，圖像標簽包裝成多合一介面，實際上是多個服務。

        圖像標簽按照服務的實際使用數量進行收費。例如一張圖片同時調用相冊版、攝像頭版兩個服務，那麽此次調用按照兩次計費。
        >
        - 公共參數中的簽名方式必須指定爲V3版本，即配置SignatureMethod參數爲TC3-HMAC-SHA256。
    ...
    def DetectMisbehavior(self, request):
        """可以識别輸入的圖片中是否包含不良行爲，例如打架鬥毆、賭博、抽煙等，可以應用於廣告圖、直播截圖、短視訊截圖等審核，減少不良行爲對平台内容質量的影響，維護健康向上的網際網路環境。
        >
        - 公共參數中的簽名方式必須指定爲V3版本，即配置SignatureMethod參數爲TC3-HMAC-SHA256。
    ...
    def DetectProductBeta(self, request):
        """商品識别- 識物版，基於人工智慧技術、海量訓練圖片、億級商品庫，可以實現全函蓋、細粒度、高準确率的商品識别和商品推薦功能。
        本服務可以識别出圖片中的主體位置、主體商品類型，函蓋億級SKU，輸出具體商品的價格、型號等詳細訊息。
        客戶無需自建商品庫，即可快速實現商品識别、拍照搜商品等功能。

        目前“商品識别- 識物版”爲内測服務，需要申請、開通後方可使用。請在[服務開通申請表](https://cloud.taifucloud.com/apply/p/y1q2mnf0vdl) 中填寫詳細訊息和需求，如果通過審核，我們将會在2個工作日内與您聯系，並開通服務。
        内測期間，本服務免費提供最高2QPS，收費模式和標準會在正式版上線前通過站内信、簡訊通知客戶。如果需要提升並發，請與我們聯系洽談。

        注意：本文件爲内測版本，僅适用於功能體驗和測試，正式業務接入請等待正式版。正式版的輸入、輸出可能會與内測版存在少量差異。
        >
        - 公共參數中的簽名方式必須指定爲V3版本，即配置SignatureMethod參數爲TC3-HMAC-SHA256。
    ...
    def RecognizeCar(self, request):
        """Top Cloud 車輛屬性識别可對汽車車身及車輛屬性進行檢測與識别，目前支援11種車身顔色、20多種車型、300多種品牌、4000多種車系+年款的識别，同時支援對車輛的位置進行檢測。如果圖片中存在多輛車，會分别輸出每輛車的車型和坐標。
        >
        - 公共參數中的簽名方式必須指定爲V3版本，即配置SignatureMethod參數爲TC3-HMAC-SHA256。
