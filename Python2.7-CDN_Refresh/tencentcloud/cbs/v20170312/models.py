...
<li>disk-id - Array of String - 是否必填：是 - 按雲盤ID過濾，每個請求最多可指定10個雲盤ID。
        :type Filters: list of Filter
        :param BeginTime: 要查詢的操作日志的起始時間，例如：“2019-11-22 00:00:00"
        :type BeginTime: str
        :param EndTime: 要查詢的操作日志的截止時間，例如：“2019-11-22 23:59:59"
        :type EndTime: str
        """
        self.Filters = None
        self.BeginTime = None
...
class DescribeSnapshotSharePermissionRequest(AbstractModel):
    """DescribeSnapshotSharePermission請求參數結構體

    """

    def __init__(self):
        """
        :param SnapshotId: 要查詢快照的ID。可通過[DescribeSnapshots](https://cloud.taifucloud.com/document/api/362/15647)查詢獲取。
        :type SnapshotId: str
        """
        self.SnapshotId = None
...
class DescribeSnapshotSharePermissionResponse(AbstractModel):
    """DescribeSnapshotSharePermission返回參數結構體

    """

    def __init__(self):
        """
        :param SharePermissionSet: 快照的分享訊息的集合
...
注意：此欄位可能返回 null，表示取不到有效值。
        ...
        :param SnapshotCount: 雲盤擁有的快照總數。
        :type SnapshotCount: int
        :param SnapshotSize: 雲盤擁有的快照總容量，單位爲MB。
        :type SnapshotSize: int
        :param BackupDisk: 雲盤因欠費銷毀或者期銷毀時， 是否使用快照備份數據的標識。True， 銷毀時創建快照進行數據備份。False 表示直接銷毀，不進行數據備份。
...
class DiskConfig(AbstractModel):
    ...
    def __init__(self):
        ...
        :param Zone: 雲硬碟所屬的[可用區](/document/product/213/15753#ZoneInfo)。
        :type Zone: str
...
class GetSnapOverviewRequest(AbstractModel):
    """GetSnapOverview請求參數結構體

    """


class GetSnapOverviewResponse(AbstractModel):
    """GetSnapOverview返回參數結構體

    """

    def __init__(self):
        """
        :param TotalSize: 用戶快照總大小
        :type TotalSize: float
        :param RealTradeSize: 用戶快照總大小（用於計費）
        :type RealTradeSize: float
        :param FreeQuota: 快照免費額度
        :type FreeQuota: float
        :param TotalNums: 快照總個數
        :type TotalNums: int
        :param RequestId: 唯一請求 ID，每次請求都會返回。定位問題時需要提供該次請求的 RequestId。
        :type RequestId: str
        """
...
class ModifyAutoSnapshotPolicyAttributeRequest(AbstractModel):
    """ModifyAutoSnapshotPolicyAttribute請求參數結構體
    ...
    def __init__(self):
        ...
        :param IsPermanent: 通過該定期快照策略創建的快照是否永久保留。FALSE表示非永久保留，TRUE表示永久保留，預設爲FALSE。
        :type IsPermanent: bool
        :param RetentionDays: 通過該定期快照策略創建的快照保留天數，該參數不可與`IsPermanent`參數沖突，即若定期快照策略設置爲永久保留，`RetentionDays`應置0。
        :type RetentionDays: int
        """
        self.AutoSnapshotPolicyId = None
        self.Policy = None
...
class ModifyAutoSnapshotPolicyAttributeResponse(AbstractModel):
    """ModifyAutoSnapshotPolicyAttribute返回參數結構體
...
class ModifyDisksChargeTypeRequest(AbstractModel):
    """ModifyDisksChargeType請求參數結構體

    """

    def __init__(self):
        """
        :param DiskIds: 一個或多個待操作的雲硬碟ID。每次請求批次雲盤上限爲100。
        :type DiskIds: list of str
        :param DiskChargePrepaid: 預付費模式，即包年包月相關參數設置。通過該參數可以指定包年包月實例的購買時長、是否設置自動續約等屬性。
        :type DiskChargePrepaid: :class:`taifucloudcloud.cbs.v20170312.models.DiskChargePrepaid`
        """
        self.DiskIds = None
        self.DiskChargePrepaid = None
...
class ModifySnapshotsSharePermissionRequest(AbstractModel):
    """ModifySnapshotsSharePermission請求參數結構體

    """

    def __init__(self):
        """
        :param AccountIds: 接收分享快照的賬號Id清單，array型參數的格式可以參考[API簡介](https://cloud.taifucloud.com/document/api/213/568)。帳號ID不同於 號，查詢用戶帳號ID請檢視[帳號訊息](https://console.cloud.taifucloud.com/developer)中的帳號ID欄。
        :type AccountIds: list of str
        :param Permission: 操作，包括 SHARE，CANCEL。其中SHARE代表分享操作，CANCEL代表取消分享操作。
        :type Permission: str
        :param SnapshotIds: 快照ID, 可通過[DescribeSnapshots](https://cloud.taifucloud.com/document/api/362/15647)查詢獲取。
        :type SnapshotIds: list of str
        """
...
class ModifySnapshotsSharePermissionResponse(AbstractModel):
    """ModifySnapshotsSharePermission返回參數結構體
...
class Placement(AbstractModel):
    ...
    def __init__(self):
        """
        :param Zone: 雲硬碟所屬的[可用區](/document/product/213/15753#ZoneInfo)。該參數也可以通過調用  [DescribeZones](/document/product/213/15707) 的返回值中的Zone欄位來獲取。
        :type Zone: str
...
class SharePermission(AbstractModel):
    ...
    def __init__(self):
        """
        :param CreatedTime: 快照分享的時間
        :type CreatedTime: str
        :param AccountId: 分享的賬號Id
        :type AccountId: str
        """
...
class Snapshot(AbstractModel):
    ...
    def __init__(self):
        ...
        :type ImageCount: int
        :param SnapshotType: 快照類型，目前該項取值可以爲PRIVATE_SNAPSHOT或者SHARED_SNAPSHOT
        :type SnapshotType: str
        :param ShareReference: 快照當前被共享數
