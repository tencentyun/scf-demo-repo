...
注意：此欄位可能返回 null，表示取不到有效值。
        ...
        :param BackupDisk: 雲盤因欠費銷毀或者期銷毀時， 是否使用快照備份數據的标識。True， 銷毀時創建快照進行數據備份。False 表示直接銷毀，不進行數據備份。
...
class GetSnapOverviewResponse(AbstractModel):
    ...
    def __init__(self):
        ...
        :param RealTradeSize: 用戶快照總大小（用于計費）
...
class ModifySnapshotsSharePermissionRequest(AbstractModel):
    ...
    def __init__(self):
        """
        :param AccountIds: 接收分享快照的賬号Id清單，array型參數的格式可以參考[API簡介](https://cloud.taifucloud.com/document/api/213/568)。帳号ID不同于 号，查詢用戶帳号ID請檢視[帳号訊息](https://console.cloud.taifucloud.com/developer)中的帳号ID欄。
