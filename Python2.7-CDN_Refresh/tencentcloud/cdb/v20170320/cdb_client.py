...
class CdbClient(AbstractClient):
    ...
    def BalanceRoGroupLoad(self, request):
        """本介面(BalanceRoGroupLoad)用於重新均衡 RO 組内實例的負載。注意，RO 組内 RO 實例會有一次資料庫連接瞬斷，請确保應用程式能重連資料庫，謹慎操作。
    ...
    def CreateDeployGroup(self, request):
        """本介面(CreateDeployGroup)用於創建放置實例的置放群組
    ...
    def DeleteBackup(self, request):
        """本介面(DeleteBackup)用於删除資料庫備份。本介面只支援删除手動發起的備份。
    ...
    def DeleteTimeWindow(self, request):
        """本介面(DeleteTimeWindow)用於删除雲資料庫實例的維護時間視窗。删除實例維護時間視窗之後，預設的維護時間窗爲 03:00-04:00，即當選擇在維護時間視窗内切換訪問新實例時，預設會在 03:00-04:00 點進行切換訪問新實例。
    ...
    def DescribeBackupDatabases(self, request):
        """本介面(DescribeBackupDatabases)用於查詢備份文件包含的庫 (已廢棄)。
    ...
    def DescribeBackupOverview(self, request):
        """本介面(DescribeBackupOverview)用於查詢用戶的備份概覽。返回用戶當前備份總個數、備份總的占用容量、贈送的免費容量、計費容量（容量單位爲位元）。
    ...
    def DescribeBackupSummaries(self, request):
        """本介面(DescribeBackupSummaries)用於查詢備份的統計情況，返回以實例爲維度的備份占用容量，以及每個實例的數據備份和日志備份的個數和容量（容量單位爲位元）。
    ...
    def DescribeBackupTables(self, request):
        """本介面(DescribeBackupTables)用於查詢指定的資料庫的備份數據表名 (已廢棄)。
        舊版本支援全量備份後，用戶如果分庫表下載邏輯備份文件，需要用到此介面。
        新版本支援(CreateBackup)創建邏輯備份的時候，直接發起指定庫表備份，用戶直接下載該備份文件即可。
    ...
    def DescribeBinlogBackupOverview(self, request):
        """本介面(DescribeBinlogBackupOverview)用於查詢用戶在當前地域總的日志備份概覽。
    ...
    def DescribeBinlogs(self, request):
        """本介面(DescribeBinlogs)用於查詢雲資料庫實例的 binlog 文件清單。
    ...
    def DescribeDBInstanceGTID(self, request):
        """本介面(DescribeDBInstanceGTID)用於查詢雲資料庫實例是否開通了 GTID，不支援版本爲 5.5 以及以下的實例。
    ...
    def DescribeDBInstances(self, request):
        """本介面(DescribeDBInstances)用於查詢雲資料庫實例清單，支援通過項目 ID、實例 ID、訪問網址、實例狀态等過濾條件來篩選實例。支援查詢主實例、災備實例和只讀實例訊息清單。
    ...
    def DescribeDataBackupOverview(self, request):
        """本介面(DescribeDataBackupOverview)用於查詢用戶在當前地域總的數據備份概覽。
    ...
    def DescribeDeployGroupList(self, request):
        """本介面(DescribeDeployGroupList)用於查詢用戶的置放群組清單，可以指定置放群組 ID 或置放群組名稱。
    ...
    def DescribeRoGroups(self, request):
        """本介面(DescribeRoGroups)用於查詢雲資料庫實例的所有的RO組的訊息。
    ...
    def DescribeRollbackTaskDetail(self, request):
        """本介面(DescribeRollbackTaskDetail)用於查詢雲資料庫實例回檔任務詳情。
    ...
    def DescribeTagsOfInstanceIds(self, request):
        """本介面(DescribeTagsOfInstanceIds)用於獲取雲資料庫實例的標簽訊息。
    ...
    def IsolateDBInstance(self, request):
        """本介面(IsolateDBInstance)用於隔離雲資料庫實例，隔離後不能通過IP和端口訪問資料庫。隔離的實例可在資源回收筒中進行開機。若爲欠費隔離，請盡快進行儲值。
    ...
    def ModifyAccountPrivileges(self, request):
        ...
        注意，修改賬號權限時，需要傳入該賬號下的全量權限訊息。用戶可以先通過 [查詢雲資料庫帳戶的權限訊息
        ](https://cloud.taifucloud.com/document/api/236/17500) 查詢該賬號下的全量權限訊息，然後進行權限修改。
    ...
    def ModifyDBInstanceVipVport(self, request):
        """本介面(ModifyDBInstanceVipVport)用於修改雲資料庫實例的IP和端口號，也可進行基礎網絡轉 VPC 網絡和 VPC 網絡下的子網變更。
    ...
    def ModifyInstanceTag(self, request):
        """本介面(ModifyInstanceTag)用於對實例標簽進行添加、修改或者删除。
    ...
    def ModifyRoGroupInfo(self, request):
        """本介面（ModifyRoGroupInfo）用於更新雲資料庫只讀組的訊息。包括設置實例延遲超限剔除策略，設置只讀實例讀權重等。
    ...
    def OfflineIsolatedInstances(self, request):
        """本介面(OfflineIsolatedInstances)用於立即下線隔離狀态的雲資料庫實例。進行操作的實例狀态必須爲隔離狀态，即通過 [查詢實例清單](https://cloud.taifucloud.com/document/api/236/15872) 介面查詢到 Status 值爲 5 的實例。
    ...
    def OpenDBInstanceGTID(self, request):
        """本介面(OpenDBInstanceGTID)用於開啓雲資料庫實例的 GTID，只支援版本爲 5.6 以及以上的實例。
    ...
    def ReleaseIsolatedDBInstances(self, request):
        """本介面（ReleaseIsolatedDBInstances）用於恢複已隔離雲資料庫實例。
    ...
    def RenewDBInstance(self, request):
        """本介面(RenewDBInstance)用於續約雲資料庫實例，支援付費模式爲包年包月的實例。按量計費實例可通過該介面續約爲包年包月的實例。
    ...
    def UpgradeDBInstance(self, request):
        """本介面(UpgradeDBInstance)用於升級或降級雲資料庫實例的配置，實例類型支援主實例、災備實例和只讀實例。
    ...
    def VerifyRootAccount(self, request):
        """本介面(VerifyRootAccount)用於校驗雲資料庫實例的 ROOT 賬號是否有足夠的權限進行授權操作。
