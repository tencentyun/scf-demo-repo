...
class InitOralProcessRequest(AbstractModel):
    ...
    def __init__(self):
        ...
        :type WorkMode: int
        :param EvalMode: 評估模式，0：詞模式（中文評測模式下爲文字模式），1：句子模式，2：段落模式，3：自由說模式，當爲詞模式評估時，能夠提供每個音節的評估訊息，當爲句子模式時，能夠提供完整度和流利度訊息。4: 英文單詞音素診斷評測模式，針對一個單詞音素診斷評測。
        :type EvalMode: int
...
class KeywordEvaluateRequest(AbstractModel):
    ...
    def __init__(self):
        ...
        :type IsEnd: int
        :param VoiceFileType: 語音文件類型 	1: raw, 2: wav, 3: mp3, 4: speex (語言文件格式目前僅支援 16k 采樣率 16bit 編碼單聲道，如有不一緻可能導緻評估不準确或失敗)。
...
class KeywordEvaluateResponse(AbstractModel):
    """KeywordEvaluate返回參數結構體

    """

    def __init__(self):
        """
        :param KeywordScores: 關鍵詞得分
...
class PhoneInfo(AbstractModel):
    ...
    def __init__(self):
        ...
        :type Stress: bool
        :param ReferencePhone: 參考音素，在單詞診斷模式下，代表标準音素
...
class TransmitOralProcessWithInitRequest(AbstractModel):
    ...
    def __init__(self):
        ...
        :param WorkMode: 語音輸入模式，0：流式分片，1：非流式一次性評估
        :type WorkMode: int
        :param EvalMode: 評估模式，0：詞模式（中文評測模式下爲文字模式），1：句子模式，2：段落模式，3：自由說模式，當爲詞模式評估時，能夠提供每個音節的評估訊息，當爲句子模式時，能夠提供完整度和流利度訊息，4：單詞糾錯模式：能夠對單詞和句子中的讀錯讀音進行糾正，給出參考正确讀音。
        :type EvalMode: int
        :param ScoreCoeff: 評價苛刻指數，取值爲[1.0 - 4.0]範圍内的浮點數，用于平滑不同年齡段的分數，1.0爲小年齡段，4.0爲最高年齡段
        :type ScoreCoeff: float
        :param SoeAppId: 業務應用ID，與賬号應用APPID無關，是用來方便客戶管理服務的參數，新的 SoeAppId 可以在[控制台](https://console.cloud.tencent.com/soe)【應用管理】下新建。
...
class WordRsp(AbstractModel):
    ...
    def __init__(self):
        """
        :param MemBeginTime: 當前單詞語音起始時間點，單位爲ms，該欄位段落模式下無意義。
        :type MemBeginTime: int
        :param MemEndTime: 當前單詞語音終止時間點，單位爲ms，該欄位段落模式下無意義。
        :type MemEndTime: int
        ...
        :param ReferenceWord: 參考詞，目前爲保留欄位。
